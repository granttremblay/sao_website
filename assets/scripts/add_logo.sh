...
#   2. ensures `.impact-logo-<name> { --logo-h: Npx; }` exists in assets/css/style_v6.css,
...
CSS="$REPO/assets/css/style_v6.css"
...
if not anchor:
    sys.exit("Could not find the .impact-logo-* knob block in assets/css/style_v6.css")
...
print("\nCSS size knobs (assets/css/style_v6.css):")
for a in added:
...
if [ "${1:-}" = "--push" ]; then
  cd "$REPO"
  git add assets/logos assets/css/style_v6.css index.html
